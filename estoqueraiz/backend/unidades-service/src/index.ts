...
import rotaUnidades from "./routes/rotaUnidades";
...
const SERVICE_NAME = "unidades-service";
const VERSION = "1.0.0";
...
app.use("/api/unidades", rotaUnidades);
...
assinanteEventos.registrarManipulador(
  ...
  async (dados) => {
    logger.info(`Novo usuario criado:`, dados);
  }
);
...
const iniciar = async () => {
  try {
    ...
    await assinanteEventos.inscrever([EventosTipo.USUARIO_CRIADO]);
