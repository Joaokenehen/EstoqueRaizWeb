...
export const BarraAcoesLote = ({ 
  ...
  return (
    <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between shadow-sm animate-in fade-in zoom-in duration-200">
      <div className="flex items-center space-x-2">
        <span className="text-red-700 font-bold text-lg">{quantidadeSelecionada}</span>
        <span className="text-red-600 font-medium text-sm">{textoItem} selecionado(s)</span>
      </div>
      <button
        onClick={onExcluir}
        disabled={carregando}
